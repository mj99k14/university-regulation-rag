...
BUCHIK_DATE_PAT = re.compile(r"[<〈](\d{4}\.\d+\.\d+\.?)[>〉]")
...
def parse_buchik(lines):
    """부칙 섹션 파싱 — 부칙별 조문을 parent-child 청크로 변환.

    부칙은 개정일마다 여러 개가 있으며, 각 부칙 내 조문(제1조, 제2조...)을
    parse_bonchik과 동일한 방식으로 청킹한다.
    chunk_id = "부칙-{시행일}-제N조" 형태로 본칙과 구분.
    """
    doc_meta = {
        "문서명": "영진전문대학교 학칙",
        "문서_코드": "2-1-1",
    ...
    def flush_jo():
        ...
        flush_hang()
        raw_text = "\n".join(jo_raw_lines)
        if not raw_text.strip() or jo_deleted:
            jo_id = None
            jo_raw_lines.clear()
            hang_list.clear()
            return
        ...
        for h_no, h_raw in hang_list:
            ...
            h_text = f"[부칙 {buchik_date}]\n" + clean_text(h_raw)
            if clean_text(h_raw).strip().startswith("삭제"):
                continue
            chunks.append({
    ...
    for line, page_num in lines:
        # 부칙 헤더 감지 (여러 부칙이 있을 수 있음)
        if BUCHIK_PAT.match(line):
            flush_jo()
            in_buchik = True
            buchik_idx += 1
            m = BUCHIK_DATE_PAT.search(line)
            buchik_date = m.group(1).rstrip(".") if m else f"부칙{buchik_idx}"
            continue
        ...
        if m:
            flush_jo()
            jo_no_raw = m.group(1)
            jo_id     = f"부칙-{buchik_date}-{jo_no_raw}"
            jo_no     = int(m.group(2))
            jo_ui     = m.group(3)
            jo_title  = m.group(4)
            jo_page   = page_num
            ...
            jo_raw_lines = [f"{jo_no_raw}({jo_title})", rest] if rest else [f"{jo_no_raw}({jo_title})"]
...
def main():
    ...
    buchik_chunks   = parse_buchik(lines)
    byeolpyo_chunks = extract_byeolpyo(PDF_PATH)

    chunks = bonchik_chunks + buchik_chunks + byeolpyo_chunks
